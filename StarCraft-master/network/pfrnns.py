...
class PFGRUCell(PFRNNBaseCell):
    ...
    def forward(self, input_, hx):
        ...
        z = torch.sigmoid(self.fc_z(torch.cat((h0, act), dim=main_x)))
       
        r = torch.sigmoid(self.fc_r(torch.cat((h0, act), dim=main_x)))
        
        n = self.fc_n(torch.cat((r * h0, act), dim=main_x))
        
        mu_n, var_n = torch.split(n, split_size_or_sections=self.h_dim, dim=main_x)
        ...
        if batch_size == 1:
            ...
            #print(n.size())
            #exit()
            n = n.view(batch_size,self.num_particles, -1, self.h_dim).transpose(0,
                                                                 1).contiguous()
            n = n.view(batch_size,self.num_particles,self.h_dim)
        n = self.batch_norm(n)
        if batch_size == 1:
            n = n.transpose(0, 1).contiguous().view(-1, self.h_dim)
        else:
            n = n.transpose(0, 1).contiguous().view(batch_size,self.num_particles, self.h_dim)
        ...
        if batch_size == 1:
            ...
            #print("LEARNING!!!")
            obs = obs.unsqueeze(1)
            obs = obs.repeat(1,15,1)
        #obs = obs.unsqueeze(0)
        att = torch.cat((h1, obs), dim=main_x)
        logpdf_obs = self.fc_obs(att)
        ...
        if batch_size != 1:
            #print(h1.size())
            #print(p1.size())
            h1 = h1.view(h1.size()[0]*h1.size()[1],h1.size()[2])
            p1 = p1.view(p1.size()[0]*p1.size()[1],p1.size()[2])
            #print(h1.size())
            #print(p1.size())
            #exit()
        #print(h1.size())
