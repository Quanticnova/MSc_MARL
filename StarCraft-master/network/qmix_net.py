...
class QMixNet(nn.Module):
    def __init__(self, args,input_size = False):
        ...
        if args.two_hyper_layers:
            ...
            self.hyper_w1 = nn.Sequential(nn.Linear(input_size, args.hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(args.hyper_hidden_dim, args.n_agents * args.qmix_hidden_dim))
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Sequential(nn.Linear(input_size, args.hyper_hidden_dim),
                                          nn.ReLU(),
                                          nn.Linear(args.hyper_hidden_dim, args.qmix_hidden_dim))
        else:
            self.hyper_w1 = nn.Linear(input_size, args.n_agents * args.qmix_hidden_dim)
            # 经过hyper_w2得到(经验条数, 1)的矩阵
            self.hyper_w2 = nn.Linear(input_size, args.qmix_hidden_dim * 1)

        # hyper_w1得到的(经验条数，args.qmix_hidden_dim)矩阵需要同样维度的hyper_b1
        self.hyper_b1 = nn.Linear(input_size, args.qmix_hidden_dim)
        # hyper_w2得到的(经验条数，1)的矩阵需要同样维度的hyper_b1
        self.hyper_b2 =nn.Sequential(nn.Linear(input_size, args.qmix_hidden_dim),
                                     nn.ReLU(),
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_shape)
        ...
        states = states.reshape(-1, self.input_size)  # (episode_num * max_episode_len, state_shape)
