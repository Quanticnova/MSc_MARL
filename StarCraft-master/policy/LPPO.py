...
class CentralV:
    def __init__(self, args):
        ...
        self.eval_critic = LRPCritic((input_shape)*self.n_agents,args)
        self.target_critic = LRPCritic((input_shape)*self.n_agents,args)
        ...
        for i in range(0, len(self.eval_critic.layers)):
            self.eval_critic.layers[i].register_forward_hook(forward_hook)
            self.target_critic.layers[i].register_forward_hook(forward_hook)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        for i in range(self.args.ppo_step):
            ...
            ratio = (log_pi_taken - old_log_pi_taken).exp()
            #print(ratio.size())
            #exit()
            ...
            #print(ratio.size())
            #exit()
            loss = -(torch.min(ratio*advantage,torch.clamp(ratio,1-0.1,1+0.1)*advantage)*mask).sum()/mask.sum()
    ...
    def _get_v_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            inputs, inputs_next = self._get_Q_tot_inputs(batch, transition_idx,max_episode_len)
            if self.args.cuda:
                inputs = inputs.cuda()
                inputs_next = inputs_next.cuda()
    ...
    def _get_actor_inputs(self, batch, transition_idx):
        ...
        if transition_idx >= self.args.stack-1:
            ...
            #preprep stack
            for i in range(self.args.stack): 
                obs_list.append(batch['o'][:,0])
                #obs_next_list.append(batch['o_next'][:,0])

            for i in range(self.args.stack-2):
                if i < transition_idx:
                    #obs_list.append(batch['o'][:,i])
                    #obs_next_list.append(batch['o_next'][:,i])
                    obs_list[self.args.stack-1-i] = batch['o'][:,transition_idx-i]
                    #obs_next_list[self.args.stack-1-i] = batch['o_next'][:,transition_idx-i]
                #else:
        ...
        #print(obs.size())
        #exit()
        obs = obs.view(obs.size()[0],self.args.n_agents,self.obs_shape)
        #obs_next = obs_next.view(self.args.batch_size,self.args.n_agents,self.obs_shape)
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        mask = (1 - batch["padded"].float()).repeat(1, 1, 1)  # 用来把那些填充的经验的TD-error置0，从而不让它们影响到学习
        if self.args.cuda:
            mask = mask.cuda()
            r = r.cuda()
            terminated = terminated.cuda()
        v_evals, v_next_target,input,input_next = self._get_v_values(batch, max_episode_len)
        ...
        for i in range(max_episode_len):
            ...
            if self.args.target_net == True:
                temp = self.target_critic(input[i])
                LRP = self.target_critic.relprop(temp)
            else:
                temp = self.eval_critic(input[i])
                LRP = self.eval_critic.relprop(temp)
            ...
            t1 = 1 - terminated[:,i,:].detach().cpu()
            b1 = temp.detach().cpu()*t1
            l1 = LRP.sum(dim=1).detach().cpu()
            t = t1.sum().detach().cpu()
            t_list.append(t)
