...
class CentralV:
    def __init__(self, args):
        ...
        #self.old_probs = torch.zeros()
        self.eval_rnn = RNN(actor_input_shape, args)
        
        print('INIT MAPPO')

        self.eval_critic = Critic(self.state_shape, self.args)
        self.target_critic = Critic(self.state_shape, self.args)
        ...
        if args.optimizer == "RMS":
            self.critic_optimizer = torch.optim.RMSprop(self.critic_parameters, lr=1e-3)
            self.rnn_optimizer = torch.optim.RMSprop(self.rnn_parameters, lr=1e-4)
        self.args = args

        # 执行过程中，要为每个agent都维护一个eval_hidden
        # 学习过程中，要为每个episode的每个agent都维护一个eval_hidden
        self.old_logs = torch.zeros(args.batch_size,args.episode_limit,args.n_agents)
        if args.cuda:
            self.old_logs = self.old_logs.cuda()
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        #action_log_prob = dist.log_prob(u)
        #print(action_log_prob.size())
        #exit()
        ...
        old_log_pi_taken = torch.log(pi_taken).detach()

        if self.epoch == 0:
            self.old_logs = old_log_pi_taken
        else:
            self.old_logs = torch.roll(self.old_logs,1,0)
            self.old_logs[0,:,:] = old_log_pi_taken[0,:,:]

        if self.args.window == False:
            self.old_logs = old_log_pi_taken
        #print(old_log_pi_taken.size())
        #exit()
        #td_error,inputs,inputs_next = self._train_critic(batch, max_episode_len, train_step)
        r = r.repeat(1, 1, self.n_agents)#.cuda()

        if self.args.cuda:
            r =r.cuda()
        #td_error = td_error.repeat(1, 1, self.n_agents)
        for i in range(self.args.ppo_step):
            
            #calculate old values under new policy
            self.init_hidden(episode_num)
            td_error,inputs,inputs_next = self._train_critic(batch, max_episode_len, train_step,i)
            ...
            # Finding the ratio (pi_theta / pi_theta__old), KL divergence
            #ratio = (log_pi_taken - old_log_pi_taken.detach()).exp()
            ratio = (log_pi_taken - self.old_logs.detach()).exp()
            #calc advantage
            ...
            loss1 = -torch.min(surr1,surr2) - 0.005*entropy
            loss = (loss1*mask).sum()/mask.sum()
            #loss = -(torch.min(ratio*advantages,torch.clamp(ratio,1-0.1,1+0.1)*advantages)*mask).sum()/mask.sum()
    ...
    def _get_v_values(self, batch, max_episode_len):
        ...
        #print(v_evals.size())
        #exit()
        return v_evals, v_targets,input_list,input_list_next
    ...
    def _train_critic(self, batch, max_episode_len, train_step,k):
        ...
        v_evals, v_next_target,inputs,inputs_next = self._get_v_values(batch, max_episode_len)
        #v_next_target = v_next_target.squeeze(-1)
        v_next_target = v_next_target.repeat(1,1,self.n_agents)
        v_evals = v_evals.repeat(1,1,self.n_agents)
        
        targets = td_lambda_target(batch, max_episode_len, v_next_target, self.args)#.unsqueeze(-1)

        if self.args.cuda:
            targets = targets.cuda()

        if k == 0:
            self.old_vals = v_evals.detach()
        
       
        advantage = (targets.detach() - v_evals.detach()).squeeze(-1)
        surr1 = v_evals - targets.detach()  #surr1
        surr2 = (self.old_vals + torch.clamp(v_evals - self.old_vals,1-self.args.clip,1+self.args.clip)) - targets.detach()
        #td_error = targets.detach() - v_evals

        #td_error = td_error.squeeze()
        #td_error = torch.min(surr1,surr2).squeeze(-1)
        td_error = surr1.squeeze(-1)
        ...
        #if k  ==  4:
            #self.old_vals = v_evals.detach()
        return advantage,inputs,inputs_next
