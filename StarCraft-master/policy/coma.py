...
class COMA:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        old_log_pi_taken = torch.log(pi_taken)
        ...
        for i in range(self.args.ppo_step):
            self.init_hidden(episode_num) 
            q_values = self._train_critic(batch, max_episode_len, train_step,i)  # 训练critic网络，并且得到每个agent的所有动作的Ｑ值  
            q_taken = torch.gather(q_values, dim=3, index=u).squeeze(3)  # 每个agent的选择的动作对应的Ｑ值 
            ...
            entropy = dist.entropy().cuda()

            # Finding the ratio (pi_theta / pi_theta__old)
            ratio = (log_pi_taken - self.old_logs.detach().detach()).exp().cuda()
            #计算advantage
            ...
            loss1 = -torch.min(surr1,surr2) - 0.001*entropy
    ...
    def _get_critic_inputs(self, batch, transition_idx, max_episode_len):
        ...
        #Get vector obs
        #s,s_next = self._get_Q_tot_inputs(batch,transition_idx,max_episode_len)
        #s = s.unsqueeze(1).expand(-1, self.n_agents, -1)
        #s_next = s_next.unsqueeze(1).expand(-1, self.n_agents, -1)
    ...
    def _train_critic(self, batch, max_episode_len, train_step,k):
        ...
        #q2 = copy.deepcopy(q_next_target)
        #q2 = q_next_target.cpu()
        #q2 = q2.cpu()
        ...
        if k == 0:
            self.old_vals = q_evals.detach()
        # get ratios
        surr1 = q_evals - targets.detach()  #surr1
        surr2 = (self.old_vals + torch.clamp(q_evals - self.old_vals,1-self.args.clip,1+self.args.clip)) - targets.detach()
    ...
    def save_model(self, train_step):
        ...
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)#print(t.size())
            #print(temp.size())
            #exit()