...
from common.utils import td_lambda_target3 as td_lambda_target
class IQL:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        #targets = self._get_returns(q_values,mask,terminated,max_episode_len)
        q_evals, q_targets = self.get_q_values(batch, max_episode_len)

        q_targets[avail_u_next == 0.0] = - 9999999
        q_targets = q_targets.max(dim=3)[0]

        #print(r.sum()-q_values.sum())
        #targets = td_lambda_target(batch, max_episode_len, q_targets.cpu(),q_values.cpu(), self.args)
        #TODO TEST WITH r - cfv
        #r =r.repeat(1,1,self.n_agents)
        #targets = (r - q_values) + self.args.gamma * q_targets * (1 - terminated)
        targets = q_values + self.args.gamma * q_targets * (1 - terminated)
        if self.args.cuda:
    ...
    def _get_critic_inputs(self, batch, transition_idx, max_episode_len):
        ...
        """
        inputs, inputs_next = [], []#print(t.size())
            #print(temp.size())
            #exit()
        inputs_train = []
