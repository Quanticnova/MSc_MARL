...
class VDN:
    def __init__(self, args):
        ...
        self.ig_net = LRPCritic((input_shape)*self.n_agents,args)
        self.target_ig_net = LRPCritic((input_shape)*self.n_agents,args)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        munchausen_reward = (LRP + self.alpha*torch.clamp(munchausen_addon, min=self.lo, max=0))

        # Compute Q targets for current states 
        Q_targets = munchausen_reward #+ entropy_targets
        q_evals = torch.gather(q_evals, dim=3, index=u).squeeze(3)
      
        td_error =   q_evals - Q_targets.detach()
