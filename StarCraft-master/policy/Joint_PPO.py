...
class CentralV:
    def __init__(self, args):
        ...
        critic_input_shape = self.obs_shape  # critic网络输入的维度
        # 根据参数决定RNN的输入维度
        if args.last_action:
    ...
    def _get_v_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            inputs, inputs_next =  batch['o'][:, transition_idx],batch['o_next'][:, transition_idx]#batch['s'][:, transition_idx], batch['s_next'][:, transition_idx],
            if self.args.cuda:
                inputs = inputs.cuda()
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        v_next_target = v_next_target.sum(dim=2)
        #print(r.size())
        #print(v_evals.size())
        #print(v_next_target.size())
        #exit()
