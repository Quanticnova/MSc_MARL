...
class CentralV:
    def __init__(self, args):
        ...
        self.old_critic = copy.deepcopy(self.eval_critic)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        #Get regret values:
        with torch.no_grad():
                old_action_prob = self._get_old_action_prob(batch, max_episode_len, epsilon)
                old_v_vals = self._get__old_v_values(batch,max_episode_len)
        self.old_critic = copy.deepcopy(self.eval_critic)
        self.old_policy = copy.deepcopy(self.eval_rnn)

        pi_taken_old = torch.gather(old_action_prob, dim=3, index=u).squeeze(3)
        pi_taken_old[mask == 0] = 1.0  # 因为要取对数，对于那些填充的经验，所有概率都为0，取了log就是负无穷了，所以让它们变成1
        log_pi_taken_old = torch.log(pi_taken_old)

        old_v_vals = old_v_vals.repeat(1, 1, self.n_agents)

        cfv = old_v_vals*log_pi_taken_old

        # 训练critic网络，并且得到每条经验的td_error, (episode_num, max_episode_len, 1)
        td_error = self._train_critic(batch, max_episode_len, train_step)
        td_error = td_error.repeat(1, 1, self.n_agents) + old_v_vals
    ...
    def _get__old_v_values(self, batch, max_episode_len):
        v_evals = []
        for transition_idx in range(max_episode_len):
            inputs, _= batch['s'][:, transition_idx], batch['s_next'][:, transition_idx],
            if self.args.cuda:
                inputs = inputs.cuda()
            # 神经网络输入的是(episode_num , state_shape)二维数据，得到的是(episode_num ， 1)二维数据
            v_eval = self.old_critic(inputs)
    ...
    def _get_old_action_prob(self, batch, max_episode_len, epsilon):
        episode_num = batch['o'].shape[0]
        avail_actions = batch['avail_u']  # coma不用target_actor，所以不需要最后一个obs的下一个可执行动作
        action_prob = []
        for transition_idx in range(max_episode_len):
            ...
            outputs, self.eval_hidden_old = self.old_policy(inputs, self.eval_hidden_old)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions)
            # 把q_eval维度重新变回(8, 5,n_actions)
            outputs = outputs.view(episode_num, self.n_agents, -1)
            prob = torch.nn.functional.softmax(outputs, dim=-1)
            action_prob.append(prob)
    ...
    def init_hidden(self, episode_num):
        ...
        self.eval_hidden_old = torch.zeros((episode_num, self.n_agents, self.args.rnn_hidden_dim))
