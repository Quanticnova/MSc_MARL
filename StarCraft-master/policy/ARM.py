...
import torch
import os
from network.base_net import CFV_RNN as RNN
...
class ARM:
    def __init__(self, args):
        ...
        self.critic_net = IGCritic(input_shape,args)
        self.target_critic_net = IGCritic(input_shape,args)
        self.eval_vdn_net = VDNNet()  # 把agentsQ值加起来的网络
        self.target_vdn_net = VDNNet()

        # Set old critic and policy to be copies of old networks
        self.old_critic = copy.deepcopy(self.critic_net)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        q_evals, q_targets,v_evals, v_targets = self.get_q_values(batch, max_episode_len)
        #targets = td_lambda_target(batch, max_episode_len, q2, self.args)

        q_targets[avail_u_next == 0.0] = - 9999999
        q_targets = q_targets.max(dim=3)[0]
        targets = td_lambda_target(batch, max_episode_len, q_targets, self.args)
        ...
        v_evals = torch.gather(v_evals, dim=3, index=u).squeeze(3)
        #q_evals = q_evals.sum(dim=2)

        old_q_evals = torch.gather(old_q_evals, dim=3, index=u).squeeze(3)
        old_v_evals = torch.gather(old_v_evals, dim=3, index=u).squeeze(3)

        if train_step == 0:
            theta = 0*old_q_evals.sum(dim=2)
        else:
            theta = torch.clamp(old_q_evals - old_v_evals,min=0).sum(dim=2) # calculate old advantage function
        ...
        #exit()
        q_targets = theta + r + self.args.gamma*n_return
        q_loss = q_evals.sum(dim=2) - q_targets
        td_error = q_loss
        #q1 = q_evals - v_evals
        #q2 = theta.sum(dim=2)
        #exit()
        ...
        target_params = self.target_critic_net.parameters()
    ...
    def get_q_values(self, batch, max_episode_len):
        ...
        return q_evals, q_targets,v_evals, v_targets
