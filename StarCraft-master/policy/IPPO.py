...
class CentralV:
    def __init__(self, args):
        ...
        self.args = args
      #calc advantage
            #advantage = torch.stack(advantages,dim=1)
        # 神经网络
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        #exit()
        if self.epoch == 0 or self.args.window == False:
            self.old_logs = old_log_pi_taken
        else:
            self.old_logs = torch.roll(self.old_logs,-1,0)
            self.old_logs[31,:,:] = old_log_pi_taken[31,:,:]
        ...
        for i in range(self.args.ppo_step):
            ...
            #logr = (log_pi_taken - self.old_logs.detach())#.exp()
            #ratio = (log_pi_taken - self.old_logs.detach())**2/2
            #ratio = (logr.exp()-1) - logr
            ratio = (log_pi_taken - self.old_logs.detach()).exp()
            #if i == self.args.ppo_step -1:
                #out = ratio.mean(dim=2)
                #out = out.mean(dim=1)
                #print(out)
