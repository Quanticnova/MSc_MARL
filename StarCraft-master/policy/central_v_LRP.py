...
class CentralV:
    def __init__(self, args):
        ...
        self.args = args
        self.ins = actor_input_shape

        

        # 神经网络
        # 每个agent选动作的网络,输出当前agent所有动作对应的概率，用该概率选动作的时候还需要用softmax再运算一次。
        if self.args.alg.find("central_v") > -1:
        ...
        self.eval_critic = LRPCritic((actor_input_shape)*self.n_agents,self.args)
        self.target_critic = LRPCritic((actor_input_shape)*self.n_agents,self.args)
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        Q_tots,inputs,v_next_target,inputs_next = self._get_v_values(batch, max_episode_len)
        ...
        for i in range(max_episode_len):
            ...
            if self.args.target_net == True:
                temp = self.target_critic(inputs[i])
                LRP = self.target_critic.relprop(temp)
            else:
                temp = self.eval_critic(inputs[i])
        ...
        Q_targets =  r + self.args.gamma * v_next_target * (1 - terminated)
        #print(Q_targets.size())
        #exit()
        Q_targets = Q_targets.repeat(1,1,self.n_agents)
        
    
        advantage = Q_targets - LRP
        

        # loss函数，(episode_num, max_episode_len, n_agents)
        loss =  -((advantage.detach() * log_pi_taken) * mask).sum() / mask.sum()
        #loss = -((LRP * log_pi_taken) * mask).sum() / mask.sum()
    ...
    def _get_v_values(self, batch, max_episode_len):
        ...
        for transition_idx in range(max_episode_len):
            ...
            inputs,inputs_next= self._get_Q_tot_inputs(batch, transition_idx,max_episode_len)  # 给obs加last_action、agent_id

            input_list.append(inputs)
            input_list_next.append(inputs_next)
            q_eval = self.eval_critic(inputs)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions)
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        if self.args.cuda:
            ...
            r = r.cuda()
            terminated = terminated.cuda()
        v_evals,_,v_next_target,_ = self._get_v_values(batch, max_episode_len)
