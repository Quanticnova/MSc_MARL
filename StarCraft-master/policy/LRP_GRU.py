...
from network.IG_Critic import LRP_GRUCritic as LRPCritic
import numpy as np
...
class VDN:
    ...
    def get_q_tot(self, batch, max_episode_len,train_step):
        ...
        for transition_idx in range(max_episode_len):
            ...
            if self.args.cuda:
                inputs = inputs.cuda()
                inputs_next = inputs_next.cuda()
                self.critic_eval_hidden = self.eval_hidden.cuda()
                self.critic_target_hidden = self.target_hidden.cuda()

            input_list.append(inputs)
            input_list_next.append(inputs_next)
            #print(inputs.size())
            q_eval = self.ig_net(inputs,self.critic_eval_hidden)  # inputs维度为(40,96)，得到的q_eval维度为(40,n_actions)
            #print(q_eval.size())
            #exit()
            q_target = self.target_ig_net(inputs_next,self.critic_target_hidden)
    ...
    def init_hidden(self, episode_num):
        ...
        self.critic_eval_hidden = torch.zeros((episode_num, 1, self.args.rnn_hidden_dim))
        self.critic_target_hidden = torch.zeros((episode_num, 1, self.args.rnn_hidden_dim))
