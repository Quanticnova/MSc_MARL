...
class CentralV:
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        #set purge ratio buffer
        if self.epoch == 0 or self.args.window == False or self.update_size == 32:
            self.old_logs = log_pi_taken
        elif self.update_size > 0:
            new_data = log_pi_taken[0:self.update_size,:,:]
            self.old_logs = torch.cat((new_data,self.old_logs),dim=0)
        ...
        self.update_size = self.idx.shape[0]
        ...
        if self.idx.shape[0] > 0:
            self.old_logs = self.old_logs[ratio <=0.5]
    ...
    def _train_critic(self, batch, max_episode_len, train_step):
        ...
        if train_step > 0 and train_step % self.args.target_update_cycle == 0:
            self.target_critic.load_state_dict(self.eval_critic.state_dict())
        #print(td_error.size())
        #td_error = td_error.repeat(1,1,3)
        #print(td_error.size())
        #exit()
        return td_error
