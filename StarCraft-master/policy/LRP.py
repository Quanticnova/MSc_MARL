    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        for i in range(max_episode_len):
            ...
            if self.args.target_net == True:
                temp = self.target_ig_net(inputs[i])
                LRP = self.target_ig_net.relprop(temp)
            else:
    ...
    def _get_inputs(self, batch, transition_idx):
        ...
        for i in range(self.args.stack):
            if i < transition_idx:
                ...
                #obs_next_list.append(batch['o_next'][:,transition_self.eval_critic.layers[i].register_forward_hook(forward_hook)idx])
