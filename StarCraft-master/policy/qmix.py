...
class QMIX:
    def __init__(self, args):
        ...
        c_obs = (input_shape-self.n_agents)*self.n_agents
        #args.state_shape = (input_shape-self.n_agents)*self.n_agents
        # 神经网络
        self.eval_rnn = RNN(input_shape, args)  # 每个agent选动作的网络
        self.target_rnn = RNN(input_shape, args)
        self.eval_qmix_net = QMixNet(args,c_obs)  # 把agentsQ值加起来的网络
        self.target_qmix_net = QMixNet(args,c_obs)
        self.args = args
        if self.args.cuda:
            self.eval_rnn.cuda()
    ...
    def learn(self, batch, max_episode_len, train_step, epsilon=None):  # train_step表示是第几次学习，用来控制更新target_net网络的参数
        ...
        o_s = torch.stack(input_list,dim = 1)
        o_s_next = torch.stack(input_list_next,dim=1)

        o_s = o_s.cuda()
        o_s_next = o_s_next.cuda()
        #print(o_s.size())
        #print(s.size())
        #exit()
        q_total_eval = self.eval_qmix_net(q_evals, o_s)
        q_total_target = self.target_qmix_net(q_targets, o_s_next)
        #exit()
        #print(r.size())
        #print(q_total_target.size())
